...
export const blogPosts: BlogPost[] = [
  {
    ...
    content: [
      "今天是 2026-03-24，我干了一件很具体的事：继续把 OpenClaw 养成一个能真干活的'虚拟程序员龙虾'。不是拿来聊天，不是拿来演示，而是直接拉进真实项目里干前端、查接口、修后端、写文章、发博客。",
      "这篇《OpenClaw 养虾实战指南》写的不是想象中的流程，而是我今天刚踩完坑、刚跑通链路、刚把文章挂到博客首页第一位之后的复盘。里面有经验，也有翻车点，都是热乎的。",
      "**一、什么是'养虾'**：我说的'养虾'，其实就是把 OpenClaw 这个调度层慢慢养成一个靠谱的虚拟程序员。它自己不是某一个模型，而是一个会分派任务、调用执行器、跟踪进度、汇总结果的总控。说白了，老板下命令，虾去摇人，Claude Code 和 Codex 去干活。",
      "以前大家用 AI，很多时候是'问一句，回一句'。但真进开发场景，这样不够。我要的是它能理解上下文、拆任务、选执行器、盯进度、最后把结果交回来。所以'养虾'的核心不是装软件，而是建立一套可重复的工作流。",
      "**二、环境准备**：我今天用的核心组合很简单，OpenClaw 负责编排，Claude Code 负责重分析和前端大块开发，Codex 负责接口对齐、补洞和快速修复。技能层面，我主要用了 `coding-agent`、`planning-with-files`、`agent-team-orchestration` 这几类能力。",
      "**三、第一个任务怎么选**：养虾最忌讳一上来就甩一个模糊的大需求，比如'帮我做个平台'。这种命令听着很爽，实际最容易翻车。正确姿势是从一个边界清晰、可验证的任务开始，让它先跑通一次闭环。",
      "**四、执行器怎么选：Claude Code vs Codex**。这一步非常关键。不是谁更强，而是谁更适合当下这个活。Claude Code 适合把一个大块前端项目从无到有拉起来，Codex 则特别适合'问题已经明确，现在开始批量修'的阶段。",
      "**五、今天的实战案例：金融平台开发完整流程**。第一段活是 Claude Code 干的，任务是开发金融平台前端管理系统。最终结果是它改了 18 个文件，写了 3500+ 行代码，而且构建验证过了。",
      "**六、前后端联调：接口对齐检查方法**。前端写完不代表结束，真实开发里最容易炸的是联调。Codex 检查发现前端调的是 `/api/system/*`，后端实际暴露的是 `/api/rbac/*`。修复内容包括补全后端缺失接口、添加 Mapper 查询方法、修复 Service 层接口定义。",
      "**七、文档编写与发布**。第三段活是把今天这套实战写成文章并发到博客。踩了两个坑：Git push 失败（凭证问题）和 TypeScript 字符串里的双引号嵌套。最后结果是文章成功显示在博客列表第一位。",
      "**八、常见问题与踩坑记录**。后台运行要用 `process log/poll` 盯状态。PowerShell 中 URL 里如果带 `&` 一定要用引号包起来。PTY 是老坑，某些环境下会表现得很怪。Git 问题除了凭证还包括权限、safe.directory 之类的事。",
      "**九、最佳实践**。Prompt 不要写成口号，要写成工单。任务要拆层，不要一把梭。验证流程必须固定下来，推荐顺序：`lint -> type check -> test -> build -> 手动点关键路径`。多 Agent 编排是加速器，但前提是你会盯。",
      "**十、总结与下一步**。今天这一轮实战让我更确定一件事：OpenClaw 真能养，而且不是当宠物养，是当生产力养。前提不是它多聪明，而是你有没有把流程搭对、把活分对、把验证盯住。下一步重点不再是'能不能写代码'，而是'能不能更稳定地理解需求、拆任务、自动验证、减少返工'。",
    ],
  },
  {
